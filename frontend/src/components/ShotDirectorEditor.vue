<template>
  <section class="section director">
    <div class="section-head">
      ...
      <div class="section-actions"><button class="btn btn-sm btn-secondary" @click="createFiveActSkeleton">创建五幕骨架</button><button class="btn btn-sm btn-secondary" @click="addShot">＋ 镜头</button><button class="btn btn-sm" :disabled="saving" @click="saveAll">{{ saving ? '保存中…' : '保存并应用到场景' }}</button></div>
    </div>
    ...
    <div v-for="(shot, index) in shots" :key="shot._key" class="card shot-card">
      ...
      <details open><summary>五段导演提示词</summary>
        ...
        <div class="workbench">
          ...
          <pre v-if="shot._stateDraft" class="prompt-preview">{{ shot._stateDraft }}</pre>
...
async function saveAll() { saving.value = true; try { const payload = shots.value.map(({ _key, _preset, _presets, _history, _busy, _startState, _endState, _stateDraft, scene_id, created_at, updated_at, order, ...s }) => s); const { data } = await api.replaceSceneShots(props.projectId, props.sceneId, payload); shots.value = data.shots.map(hydrate); toast.success('五幕镜头已保存，并应用到场景生成提示词') } catch (e) { toast.error(e.response?.data?.error || '保存镜头失败') } finally { saving.value = false } }
async function applyStyle(shot) { try { if (!shot.id) { toast.error('请先保存镜头'); return } const preview = await api.applyShotStylePreset(props.projectId, shot.id, shot._preset, true); const p = preview.data.preset; if (!window.confirm(`将追加：${p.prompt_tail || '无'}\n负向词：${p.negative_tail || '无'}\n原因：${p.reason || preview.data.metadata?.preset_name || ''}\n是否应用？`)) return; const { data } = await api.applyShotStylePreset(props.projectId, shot.id, shot._preset, false); shot.prompt_style = data.prompt_style; shot.negative_prompt = data.negative_prompt || ''; await loadHistory(shot); toast.success(data.changed ? '已应用风格预设并记录来源' : '该预设已应用，无需重复追加') } catch (e) { toast.error(e.response?.data?.error || '应用预设失败') } }
async function buildStatePrompt(shot) { shot._busy = true; try { const { data } = await api.shotStatePromptDraft(props.projectId, shot.id, { start_state: shot._startState, end_state: shot._endState }); shot._stateDraft = JSON.stringify(data.draft, null, 2); toast.success('已生成起止状态草稿（未自动保存）') } catch (e) { toast.error(e.response?.data?.error || '生成起止状态草稿失败') } finally { shot._busy = false } }
