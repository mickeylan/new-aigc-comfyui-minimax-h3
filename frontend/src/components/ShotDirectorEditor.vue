    ...
    <div v-for="(shot, index) in shots" :key="shot._key" class="card shot-card">
      ...
      <details open><summary>五段导演提示词</summary>
        ...
        <div class="workbench">
          ...
          <div v-if="shot._history?.length" class="history"><button v-for="v in shot._history" :key="v.id" class="history-item" :disabled="shot._busy" @click="rollbackVersion(shot, v)">{{ actionLabel(v.action) }} · {{ v.state === 'applied' ? '已应用' : '草稿' }} · {{ new Date(v.created_at).toLocaleString() }}</button></div>
        </div>
      </details>
    </div>
  </section>
...
const blank = (actType = 'setup') => ({ _key: `new-${++key}`, _preset: '', _history: [], act_type: actType, shot_type: '中景', camera_angle: '平视', camera_movement: '固定', transition_type: '', transition_note: '', duration: 2, description: '', dialogue: '', emotion: '', prompt_subject: '', prompt_action: '', prompt_camera: '', prompt_lighting: '', prompt_style: '', negative_prompt: '' })
...
const actionLabel = a => ({ build: '构建', optimize: '优化', translate: '翻译', manual: '手工保存', rollback: '回滚草稿' }[a] || a)
