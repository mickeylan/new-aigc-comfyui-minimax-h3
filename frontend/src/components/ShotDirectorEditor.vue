<template>
  <section class="section director">
    <div class="section-head">
      ...
      <div class="section-actions"><button class="btn btn-sm btn-secondary" @click="createFiveActSkeleton">创建五幕骨架</button><button class="btn btn-sm btn-secondary" @click="addShot">＋ 镜头</button><button class="btn btn-sm" :disabled="saving" @click="saveAll">{{ saving ? '保存中…' : '保存导演设计' }}</button></div>
    </div>
    ...
    <div v-for="(shot, index) in shots" :key="shot._key" class="card shot-card">
      ...
      <div class="shot-grid two"><label>起始状态<input v-model="shot._startState" class="input" placeholder="可选，用于起止状态Skill草稿" /></label><label>结束状态<input v-model="shot._endState" class="input" placeholder="可选，用于下一镜连续性" /></label></div>
      ...
      <details open><summary>五段导演提示词</summary>
        ...
        <div class="workbench">
          ...
          <div v-if="shot._stateDraft" class="workbench"><pre class="prompt-preview">{{ JSON.stringify(shot._stateDraft, null, 2) }}</pre><button class="btn btn-sm" @click="applyStateDraft(shot)">应用到本镜五段（仍需保存）</button></div>
...
const emit = defineEmits(['scene-changed'])
...
const blank = (actType = 'setup') => ({ _key: `new-${++key}`, _preset: '', _history: [], _startState: '', _endState: '', _stateDraft: '', act_type: actType, shot_type: '中景', camera_angle: '平视', camera_movement: '固定', transition_type: '', transition_note: '', duration: 2, description: '', dialogue: '', emotion: '', prompt_subject: '', prompt_action: '', prompt_camera: '', prompt_lighting: '', prompt_style: '', negative_prompt: '' })
const hydrate = s => ({ ...s, _key: `shot-${s.id}`, _preset: '', _history: [], _presets: [], _startState: '', _endState: '', _stateDraft: '' })
...
async function saveAll() { saving.value = true; try { const payload = shots.value.map(({ _key, _preset, _presets, _history, _busy, _startState, _endState, _stateDraft, scene_id, created_at, updated_at, order, ...s }) => s); const { data } = await api.replaceSceneShots(props.projectId, props.sceneId, payload); shots.value = data.shots.map(hydrate); emit('scene-changed'); toast.success('导演镜头已保存，场景事实保持不变，旧生成结果已失效') } catch (e) { toast.error(e.response?.data?.error || '保存镜头失败') } finally { saving.value = false } }
