    ...
    <div v-for="(shot, index) in shots" :key="shot._key" class="card shot-card">
      ...
      <details open><summary>五段导演提示词</summary>
        ...
        <div class="workbench">
          ...
          <div class="tool-row">
            <select v-model="shot._preset" class="input preset-select"><option value="">选择风格预设</option><option v-for="p in presets" :key="p.id" :value="p.id">{{ p.name }} · {{ p.reason }}</option></select>
          ...
          <div v-if="shot._history?.length" class="history"><button v-for="v in shot._history" :key="v.id" class="history-item" @click="restoreVersion(shot, v)">{{ actionLabel(v.action) }} · {{ new Date(v.created_at).toLocaleString() }}</button></div>
        </div>
      </details>
    </div>
  </section>
...
const props = defineProps({ projectId: { type: [String, Number], required: true }, sceneId: { type: [String, Number], required: true } })
...
const hydrate = s => ({ ...s, _key: `shot-${s.id}`, _preset: '', _history: [] })
...
async function saveAll() { saving.value = true; try { const payload = shots.value.map(({ _key, _preset, _history, _negative, _busy, id, scene_id, created_at, updated_at, order, ...s }) => s); const { data } = await api.replaceSceneShots(props.projectId, props.sceneId, payload); shots.value = data.shots.map(hydrate); toast.success('五幕镜头已保存，并应用到场景生成提示词') } catch (e) { toast.error(e.response?.data?.error || '保存镜头失败') } finally { saving.value = false } }
async function applyStyle(shot) { try { const { data } = await api.applyPreset(shot._preset, assembledPrompt(shot)); shot.prompt_style = [shot.prompt_style, presets.value.find(p => p.id === Number(shot._preset))?.prompt_tail].filter(Boolean).join(', '); shot._negative = data.negative_prompt; toast.success('已应用风格预设') } catch (e) { toast.error(e.response?.data?.error || '应用预设失败') } }
async function expandShot(shot) { shot._busy = true; try { const { data } = await api.optimizePrompt({ project_id: Number(props.projectId), entity_type: 'shot', entity_id: shot.id || 0, prompt: `请依据简单说明扩写单个分镜：${shot.description}`, context: `叙事幕：${actLabel(shot.act_type)}；必须严格输出五行，依次为主体与五官；姿态与动作；摄影机与构图；光线与氛围；视觉风格与质感。每行只写对应内容，不要编号、标题或解释。` }); const parts = data.prompt.split(/\n|；|;/).map(v => v.replace(/^\s*\d+[.、:)：-]?\s*/, '').trim()).filter(Boolean); if (parts.length < 5) throw new Error('AI 未按五段结构返回，请重试'); [shot.prompt_subject, shot.prompt_action, shot.prompt_camera, shot.prompt_lighting, shot.prompt_style] = [parts[0], parts[1], parts[2], parts[3], parts.slice(4).join(', ')]; toast.success('镜头五段提示词已扩写') } catch (e) { toast.error(e.response?.data?.error || e.message || '镜头扩写失败') } finally { shot._busy = false } }
...
function restoreVersion(shot, version) { setPrompt(shot, version.content); toast.success('已载入历史版本，保存镜头后生效') }
