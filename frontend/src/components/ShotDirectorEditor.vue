    ...
    <div v-for="(shot, index) in shots" :key="shot._key" class="card shot-card">
      ...
      <details open><summary>五段导演提示词</summary>
        ...
        <div class="workbench">
          ...
          <div class="tool-row">
            <select v-model="shot._preset" class="input preset-select"><option value="">选择风格预设</option><option v-for="p in presets" :key="p.id" :value="p.id">{{ p.name }} · {{ p._match_reason || p.reason }}</option></select>
...
async function load() { loading.value = true; try { const [{ data }, presetRes] = await Promise.all([api.sceneShots(props.projectId, props.sceneId), api.presetRecommendations({ genre: props.genre, tone: props.tone, scene_type: props.sceneTitle, tags: props.sceneContent, limit: 5 })]); shots.value = (data.shots || []).map(hydrate); presets.value = (presetRes.data || []).map(r => ({ ...r.preset, _match_reason: (r.reasons || []).join('；'), _score: r.score })) } catch (e) { toast.error(e.response?.data?.error || '加载镜头失败') } finally { loading.value = false } }
