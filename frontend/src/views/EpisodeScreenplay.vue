  ...
  <div class="page screenplay-page">
    ...
    <section class="creative-actions card">
      <div>
        ...
        <p v-if="rollingRequired">这是 {{ project?.episodes }} 集长篇项目，必须按每批5–20集滚动规划，禁止一次生成全剧方案。</p>
...
const rollingRequired = computed(() => Number(project.value?.episodes || 0) > 20)
...
async function generateCreativePlan() {
  if (rollingRequired.value) { toast.info('长篇项目请进入滚动规划，按每批5–20集生成'); return }
...
<style scoped>
.screenplay-page{max-width:1180px;margin:0 auto;padding-bottom:90px;color:var(--text)}.screenplay-head{display:flex;justify-content:space-between;gap:24px;align-items:flex-end;margin-bottom:20px}.screenplay-head h1{margin:8px 0}.head-actions,.row-actions,.add-blocks{display:flex;gap:8px;align-items:center;flex-wrap:wrap}.creative-actions{display:flex;justify-content:space-between;gap:20px;align-items:center;margin-bottom:14px;border-left:4px solid var(--indigo)}.creative-actions p,.plan-progress span,.plan-progress p{margin:5px 0 0;color:var(--text-secondary)}.plan-progress{margin-bottom:14px;border-left:4px solid var(--indigo)}.plan-progress>div:first-child{display:flex;justify-content:space-between;gap:12px}.progress-track{height:9px;margin-top:10px;overflow:hidden;border-radius:99px;background:var(--chip-bg)}.progress-track span{display:block;height:100%;background:linear-gradient(90deg,var(--indigo),var(--teal));transition:width .8s ease}.pipeline{display:flex;gap:10px;align-items:center;flex-wrap:wrap;margin-bottom:20px}.pipeline span{padding:5px 9px;border-radius:999px;background:var(--chip-bg);color:var(--text-secondary)}.pipeline .active{color:#fff;background:var(--indigo)}.pipeline .btn{margin-left:auto}.scene-list{display:grid;gap:18px}.scene-card{display:grid;grid-template-columns:64px 1fr;padding:0;overflow:hidden;background:var(--card-solid);color:var(--text)}.scene-index{padding:24px 12px;background:var(--accent-soft);color:var(--accent);font-size:22px;font-weight:700;text-align:center;border-right:1px solid var(--border)}.scene-body{padding:22px}.scene-toolbar,.block-head{display:flex;justify-content:space-between;gap:12px;align-items:center}.block-label{font-size:12px;color:var(--text-secondary);text-transform:uppercase;letter-spacing:.08em}.heading-input{font-weight:700;font-size:17px;margin:8px 0 18px}.script-block{border-left:3px solid var(--text-tertiary);padding:10px 0 10px 14px;margin:10px 0}.block-dialogue{border-color:var(--indigo);margin-left:12%}.block-character{border-color:var(--orange);margin-left:20%;max-width:55%}.block-parenthetical{border-color:var(--text-tertiary);margin-left:17%;max-width:65%}.block-transition{border-color:var(--green);margin-left:35%}.block-type{background:var(--card-solid);color:var(--text);border:1px solid var(--border);border-radius:8px;padding:5px 8px;font-weight:700}.block-type option{background:var(--card-solid);color:var(--text)}.block-text{min-height:86px;margin-top:8px}.character-input,.parenthetical-input,.transition-input{margin-top:8px}.add-blocks{padding-top:10px;border-top:1px solid var(--border)}.add-scene{display:block;margin:20px auto}.empty{text-align:center;padding:50px}.save-bar{position:fixed;left:50%;bottom:20px;transform:translateX(-50%);z-index:10;display:flex;gap:20px;align-items:center;padding:12px 18px;border:1px solid var(--border);border-radius:12px;background:var(--card-solid);color:var(--text);box-shadow:var(--shadow-hover)}.back{display:block;margin-bottom:8px}@media(max-width:760px){.screenplay-head{align-items:stretch;flex-direction:column}.scene-card{grid-template-columns:42px 1fr}.scene-body{padding:14px}.block-dialogue,.block-character,.block-parenthetical,.block-transition{margin-left:0;max-width:none}.pipeline b{display:none}}
</style>
