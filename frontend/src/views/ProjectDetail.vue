...
const videoCatalog = computed(() => {
  ...
  return rows.length ? rows : [
    { template_code: 'minimax_h3_ref2v', name: '多图参考生视频', max_references: 9 },
    { template_code: 'minimax_h3_i2v', name: '图生视频', max_references: 1 },
    { template_code: 'minimax_h3_first_last', name: '首尾帧生视频', max_references: 2 }
  ]
})
...
async function uploadVideoFrame(kind,event) {
  const file=event.target.files?.[0];event.target.value='';if(!file||!videoTaskScene.value)return
  try { const {data}=await api.uploadSceneVideoFrame(id(),videoTaskScene.value.id,kind,file);if(kind==='first')videoTaskFirstFrame.value=data.file;else videoTaskLastFrame.value=data.file;toast.success(kind==='first'?'已选择新的首帧图片':'已选择新的尾帧图片') }
  catch(e){toast.error(e.response?.data?.error||'上传帧图片失败')}
}
...
async function load() {
  try {
    const [{ data }, catalogRes] = await Promise.all([api.project(id()), api.modelCatalog()])
    modelCatalog.value = catalogRes.data?.items || []
    loadError.value = ''
    project.value = data.project
