...
func (s *ProjectService) UpdateProject(p *models.Project, req models.Project) error {
	if req.Episodes < 1 || req.Episodes > 100000 {
		return fmt.Errorf("全剧预计集数必须在 1 到 100000 之间")
	}
...
除结构化Dialogue原文外，输出必须全部使用简洁自然的英文。使用官方 [Shot 1] 和 [Shot N] At MM:SS.mmm, 控制格式；不得翻译、复述或自行补写对白。
	...
	actionTrusted := func(value string, allowSubjects bool) bool {
		...
		if !allowSubjects {
			for _, name := range parseSceneCharacters(sc.Characters) {
				if strings.Contains(shotContext, name) && !strings.Contains(value, name) {
					return false
				}
			}
		}
		...
		if allowSubjects {
			for _, match := range h3SubjectTagPattern.FindAllStringSubmatch(value, -1) {
				n, _ := strconv.Atoi(match[1])
				if n < 1 || n > len(refLines) {
					return false
				}
			}
		}
	...
	for i, shot := range shots {
		...
		for _, field := range structured {
			if clean := conciseVisibleShotField(field); clean != "" {
				fields = append(fields, englishH3ShotField(clean))
			}
		}
...
func renderDialogueRange(d models.Dialogue, speakerID int, referenceLines []string, fragment string, continuesFrom, continuesTo, visible bool, visibleSubjects []string) string {
...
func isCharacterSubjectTag(tag string, referenceLines []string) bool {
	...
	n, _ := strconv.Atoi(match[1])
	return n >= 1 && n <= len(referenceLines) && strings.Contains(referenceLines[n-1], "角色「")
}

func ensureVisibleDialogueSpeakers(segment string, _ []string) string {
...
func appendExplicitDialogueRangesToShots(body string, dubs []models.Dialogue, referenceLines []string, shots []models.Shot) (string, bool) {
	...
	for i, shot := range shots {
		for _, r := range shot.DialogueRanges {
			...
			framing := useSubjectTags(strings.Join([]string{shot.PromptSubject, shot.ShotType}, " "), referenceLines)
			speakerTag := useSubjectTags(strings.TrimSpace(d.Character), referenceLines)
			visible := speakerTag != "" && strings.Contains(framing, speakerTag)
			...
			for _, tag := range h3SubjectTagPattern.FindAllString(framing, -1) {
				if !seenSubject[tag] {
...
func appendStructuredDialogueToShots(body string, dubs []models.Dialogue, referenceLines []string, shots []models.Shot) string {
	...
	for shotNo := 1; shotNo <= len(shots); shotNo++ {
		for _, index := range assigned[shotNo] {
			...
			framing := useSubjectTags(strings.Join([]string{shots[shotNo-1].PromptSubject, shots[shotNo-1].ShotType}, " "), referenceLines)
			...
			byShot[shotNo] = append(byShot[shotNo], renderDialogueRange(valid[index], speakerIDs[index], referenceLines, valid[index].Text, continuation, index+1 < len(valid) && dialogueContinuesAcrossCut(valid[index], valid[index+1]) && dialogueIndexShot(assigned, index+1) == shotNo+1, visible, subjects))
		}
	}
...
func normalizeSavedH3Audio(prompt string, dubs []models.Dialogue, referenceLines []string, shotSets ...[]models.Shot) string {
	...
	detail = regexp.MustCompile(`(?i)The same voice continues uninterrupted across the cut\.`).ReplaceAllString(detail, "")
	if len(shotSets) > 0 {
...
func h3VideoSubjects(referenceLines []string) (definitions, retention, subjectRefs []string) {
	...
	for i, line := range referenceLines {
		...
		english := englishH3ReferenceDescription(desc, n)
		definitions = append(definitions, fmt.Sprintf("<Subject %d> is %s from <Picture %d>.", n, english, n))
		trait := "identity and visible appearance"
		if strings.Contains(line, "场景「") || strings.Contains(line, "环境「") {
			trait = "environment and spatial layout"
		} else if strings.Contains(line, "分镜画面") {
			trait = "composition and action state"
		}
		retention = append(retention, fmt.Sprintf("<Subject %d> (where explicitly shown): fully_preserved - %s.", n, trait))
	}
...
func buildMiniMaxH3RefPrompt(sc *models.Scene, p *models.Project, dubs []models.Dialogue, referenceLines []string) string {
	...
	if style = h3EnglishStyle(style); style != "" {
		body = strings.Replace(body, "[Shot 1]", "The target video uses a "+style+" style.\n[Shot 1]", 1)
	}
...
func validateGeneratedH3Prompt(prompt, template string, duration float64) []string {
	...
	if regexp.MustCompile(`\[Shot\s+[0-9]+\s*\|`).MatchString(detail) {
		issues = append(issues, "Shot时间范围格式不符合官方文档")
	}
	if strings.Contains(detail, "[Shot 1] At ") {
		issues = append(issues, "Shot 1不得带时间戳")
	}
	...
	for _, marker := range h3ShotMarkerPattern.FindAllStringSubmatch(detail, -1) {
		...
		match := official[n]
		if len(match) != 5 {
			issues = append(issues, fmt.Sprintf("Shot %d缺少官方At MM:SS.mmm切入时间", n))
			continue
		}
