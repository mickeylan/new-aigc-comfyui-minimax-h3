...
func (s *Service) HandlePreviewShotMaterialization(c *gin.Context) {
	p, ok := s.loadProject(c)
	if !ok {
		return
	}
	...
	preview, err := s.Shots.PreviewMaterialization(p.ID, uint(sceneID))
	if err != nil {
		c.JSON(400, gin.H{"error": err.Error()})
		return
	}
...
func (s *Service) HandleMaterializeShots(c *gin.Context) {
	p, ok := s.loadProject(c)
	if !ok {
		return
	}
	...
	if err := c.ShouldBindJSON(&req); err != nil || !req.Confirm {
		c.JSON(400, gin.H{"error": "必须明确确认拆成可制作场景"})
		return
	}
	scenes, err := s.Shots.Materialize(p.ID, uint(sceneID))
	if err != nil {
		c.JSON(400, gin.H{"error": err.Error()})
		return
	}
