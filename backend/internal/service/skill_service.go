...
func (s *SkillService) InitSystemSkills() error {
	systemSkills := []models.Skill{
		...
		{Name: "AI完整Scene导演方案", Code: "director-scene-draft", Version: 1, Description: "根据Scene事实和用户概况生成可审核的完整Shot导演草稿", Stage: models.SkillStageStoryboard, PromptTemplate: "Scene权威事实：{{scene_facts}}\n资产上下文：{{asset_context}}\n用户概况：{{brief}}\n补充要求：{{requirements}}\n目标总时长：{{target_duration}}秒", SystemPrompt: "只输出严格JSON对象，根字段只能为shots。shots每项必须且只能包含act_type,shot_type,camera_angle,camera_movement,duration,description,dialogue,emotion,transition_type,transition_note,start_state,end_state,prompt_subject,prompt_action,prompt_camera,prompt_lighting,prompt_style,negative_prompt,checks。act_type仅限setup/rising/midpoint/falling/resolution；transition_type仅限空字符串/cut/dissolve/fade/wipe/match_cut。每镜一个主要动作，完整生成起止状态和五段导演提示词；结构化对白逐字保留，不得把动作写成对白或旁白；不新增剧情人物或关键道具。checks为该镜自动检查说明数组。", IsSystem: true, Enabled: true, SortOrder: 29},
		...
		{Name: "Qwen-Image-2.1 文生图提示词", Code: QwenImage21T2IProgram, Operation: QwenImage21T2IProgram, Version: 2, Description: "把任意长度需求扩写为完整英文观察式画面描述，覆盖空间、材质、文字、光线与独立画幅", Stage: models.SkillStageQwenImageT2I, PromptTemplate: "{{request}}", SystemPrompt: qwenImage21T2ISystem, IsSystem: true, Enabled: true, SortOrder: 1},
		{Name: "Qwen-Image-2.1 多图编辑提示词", Code: QwenImage21EditProgram, Operation: QwenImage21EditProgram, Version: 2, Description: "区分保守编辑与参考造景，绑定有序 <imageN> 的画布、身份、素材职责及保留项", Stage: models.SkillStageQwenImageEdit, PromptTemplate: "{{request}}", SystemPrompt: qwenImage21EditSystem, IsSystem: true, Enabled: true, SortOrder: 1},
		{Name: "Qwen-Image-2.1 角色设定卡", Code: QwenImage21CharacterCardProgram, Operation: QwenImage21CharacterCardProgram, Version: 1, Description: "独立生成专业角色设定卡提示词，锁定角色DNA、三视图、表情、服装鞋履配饰、武器与灵兽结构，不影响Krea2四视图", Stage: models.SkillStageQwenCharacterCard, PromptTemplate: "{{request}}", SystemPrompt: qwenImage21CharacterCardSystem, IsSystem: true, Enabled: true, SortOrder: 1},
	}
...
func (s *SkillService) GetAvailableStages() []map[string]string {
	return []map[string]string{
		...
		{"value": models.SkillStageQwenImageT2I, "label": "Qwen Image 文生图提示词"},
		{"value": models.SkillStageQwenImageEdit, "label": "Qwen Image 多图编辑提示词"},
		{"value": models.SkillStageQwenCharacterCard, "label": "Qwen Image 角色设定卡"},
	}
}
